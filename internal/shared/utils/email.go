...
func renderAuthCodeStyleEmail(heading, message, codeHTML, footerNote string) string {
	footer := ""
	if footerNote != "" {
		footer = fmt.Sprintf(`<p style="color: #94a3b8; font-size: 13px; text-align: center; margin: 0;">%s</p>`, footerNote)
	}
...
<body style="font-family: Inter, Arial, sans-serif; background: #f8fafc; padding: 40px 0;">
  <div style="max-width: 480px; margin: 0 auto; background: #fff; border-radius: 16px; border: 1px solid #e2e8f0; overflow: hidden;">
    ...
    <div style="padding: 40px 32px;">
      <h2 style="color: #1e293b; font-size: 20px; font-weight: 700; margin: 0 0 8px;">%s</h2>
      <p style="color: #64748b; font-size: 15px; margin: 0 0 32px;">%s</p>
      <div style="background: #f1f5f9; border-radius: 12px; padding: 24px; text-align: center; color: #1e293b; margin-bottom: 32px;">
        %s
      </div>
      %s
    </div>
...
</html>`, heading, message, codeHTML, footer)
}
